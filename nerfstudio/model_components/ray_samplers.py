...
class ProposalNetworkSampler(Sampler):
    ...
    def generate_ray_samples(
        ...
        for i_level in range(n + 1):
            ...
            if is_prop:
                if updated:
                    ...
                    if self.use_sdf:
                        if i_level == n - 1:
                                density, sdf = density_fns[i_level](ray_samples.frustums.get_positions(), return_sdf=return_sdf)
                        else:
        ...
        if return_sdf:
            return ray_samples, weights_list, ray_samples_list, sdf
...
class NeuSSampler(Sampler):
    ...
    def generate_ray_samples(
        ...
        while total_iters < self.num_upsample_steps:
            ...
            if use_laplace_density:
                alpha = base_variance * 2**total_iters
                beta = 1.0 / alpha
                density = alpha * (0.5 + 0.5 * sdf.sign() * torch.expm1(-sdf.abs() / beta))
                weights = ray_samples.get_weights(density.view(*ray_samples.frustums.directions.shape[:-1], -1))
            else:
