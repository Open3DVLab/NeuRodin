...
def setup_optimizers(config: base_config.Config, param_groups: Dict[str, List[Parameter]]) -> "Optimizers":
    ...
    #######
    if config.method_name in ['pure-neus', 'hydoor']:
        if not hasattr(config.pipeline.model, 'use_proposal_network_sampler') or not config.pipeline.model.use_proposal_network_sampler:
            del optimizer_config['proposal_networks']
        if config.pipeline.model.sdf_field.use_grid_feature == False:
            del optimizer_config['encoding']
    ######
...
class Optimizers:
    ...
    def __init__(self, config: Dict[str, Any], param_groups: Dict[str, List[Parameter]]):
        ...
        #######
        total_params = 0
        for param_group_name, params_group in param_groups.items():
            for params in params_group:
                total_params += params.numel()
        print('>>> Total trainable parameters number: ', total_params)
        #######
