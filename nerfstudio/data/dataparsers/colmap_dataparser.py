...
MAX_AUTO_RESOLUTION = 9999999999
...
class ColmapDataParser(DataParser):
    ...
    def get_train_eval_split_filename(self, image_filenames, split_path):              ##########
        split = load_from_json(split_path)
        train_split_names = split['train']
        test_split_names = split['test']
        i_train = [i for i, path in enumerate(image_filenames) if path.name.split('.')[0] in train_split_names]
        i_test = [i for i, path in enumerate(image_filenames) if path.name.split('.')[0] in test_split_names]
        print('train set length:', len(i_train))
        print('test set length:', len(i_test))

        return np.array(i_train), np.array(i_test)
    ...
    def _get_image_indices(self, image_filenames, split):
        ...
        if (self.config.data / f"{split}_list.txt").exists():
            ...
            if self.config.eval_mode == "fraction":
                ...
                i_train, i_eval = self.get_train_eval_split_filename(image_filenames, self.config.split_path)                    ##############
    ...
    def _generate_dataparser_outputs(self, split: str = "train", **kwargs):
        ...
        if str(image_filenames[0])[-4:] in ['.jpg', '.JPG']:
            try:
            ...
            try:
                tmp_img = Image.open(str(image_filenames[0])[:-4]+'.PNG')
                if str(image_filenames[0])[-3:] == 'png':
                    image_filenames = [Path(str(fname)[:-4]+'.PNG') for fname in image_filenames]
            except:
                tmp_img = Image.open(str(image_filenames[0])[:-4]+'.png')
                if str(image_filenames[0])[-3:] == 'PNG':
                    image_filenames = [Path(str(fname)[:-4]+'.png') for fname in image_filenames]
