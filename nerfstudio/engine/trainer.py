...
class Trainer:
    ...
    def train(self) -> None:
        ...
        with TimeWriter(writer, EventName.TOTAL_TRAIN_TIME):
            ...
            log = ''
            log += f'#### Run at {time.ctime(start_time)} ####\n'
            print(f'#### Run at {time.ctime(start_time)} ####')
            #########
            ...
            ##########
            end_time = time.time()
            print(f'#### End at {time.ctime(end_time)} ####')
            log += f'#### End at {time.ctime(end_time)} ####\n'
            print(f'Elapsed time: {end_time - start_time: .2f} s')
            log += f'Elapsed time: {end_time - start_time: .2f} s\n'
            with open(str(self.base_dir / 'log.txt'), "w") as file:
                file.write(log)
            ##########
    ...
    def _load_checkpoint(self) -> None:
        ...
        if load_dir is not None:
            ...
            if self.config.trainer.is_continue:
                self._start_step = loaded_state["step"] + 1                       ########## commited by me
                # load the checkpoints for pipeline, optimizers, and gradient scalar
                self.pipeline.load_pipeline(loaded_state["pipeline"])
                self.optimizers.load_optimizers(loaded_state["optimizers"])           ########### commited by me
                if "schedulers" in loaded_state and self.config.trainer.load_scheduler:
                    self.optimizers.load_schedulers(loaded_state["schedulers"])  ########### commited by me
                self.grad_scaler.load_state_dict(loaded_state["scalers"])     ########### commited by me
            else:
                self.pipeline.load_pipeline(loaded_state["pipeline"])
