...
def collate_nearby_image_dataset_batch(batch: Dict, num_rays_per_batch: int, nearby_num: int=10 , keep_full_image: bool = False):
    """
    ...
    ## make sure image_idx is sorted!
    # first randomly sample one reference image
    sample_idx = random.sample(range(num_images), k=1)
    # then find the frames window
    frames_win = torch.tensor(range(sample_idx[0], sample_idx[0]+nearby_num)).to(device) % num_images
    num_images = frames_win.shape[0]
...
def collate_image_dataset_batch(batch: Dict, num_rays_per_batch: int, keep_full_image: bool = False, num_image_to_sample: int = -1, downsample_factor: int = 1):
    """
    Revised by Yifan. Now can choose to sample on one image.                        ####################
    ...
    device = batch["image"].device
    num_images, image_height, image_width, _ = batch["image"].shape
    if num_image_to_sample > 0:
        image_indices = random.sample(range(num_images), k=num_image_to_sample)
        k = num_image_to_sample
    else:
        k = num_images
    ...
    if "mask" in batch:
        ...
        indices = torch.floor(
            torch.rand((num_rays_per_batch, 3), device=device)
            # * torch.tensor([k, image_height, image_width], device=device)
            * torch.tensor([k, image_height//downsample_factor, image_width//downsample_factor], device=device)             ############
        ).long()
        indices[:, 1:] =  indices[:, 1:]*downsample_factor                   ##########

    c, y, x = (i.flatten() for i in torch.split(indices, 1, dim=-1))
    if num_image_to_sample > 0:
        c = torch.tensor([image_indices[i] for i in c]).to(device)
...
class PixelSampler:  # pylint: disable=too-few-public-methods
    ...
    def sample(self, image_batch: Dict, num_images_to_sample_pixel_from: int = -1, downsample_factor: int = 1):                 ######## add `num_images_to_sample_pixel_from` and `downsample_factor` by Yifan
        ...
        if isinstance(image_batch["image"], list):
            ...
            pixel_batch = collate_image_dataset_batch(
                image_batch, self.num_rays_per_batch, keep_full_image=self.keep_full_image, num_image_to_sample=num_images_to_sample_pixel_from, downsample_factor=downsample_factor             ############
            )
        else:
