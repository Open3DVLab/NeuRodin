...
class SphereColliderWithBackground(SceneCollider):
    ...
    def forward(self, ray_bundle: RayBundle) -> RayBundle:
        ...
        sphere_intersections = sphere_intersections.relu_()
        outside = sphere_intersections[:, 0:1].isnan()

        ray_bundle.nears = sphere_intersections[:, 0:1]
        ray_bundle.fars = sphere_intersections[:, 1:2]
        ray_bundle.nears[outside], ray_bundle.fars[outside] = 1, 1.2  # Dummy distances (1, 1.2). Density will be set to 0.
        ray_bundle.outside = outside
        return ray_bundle
