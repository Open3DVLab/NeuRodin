...
class Cameras(TensorDataclass):
    ...
    def _generate_rays_from_coords(
        ...
        cam_coord_directions = directions_stack[0] / torch.linalg.norm(directions_stack[0], dim=-1, keepdims=True)         ############
        ...
        return RayBundle(
            ...
            times=times,
            metadata={'ray_cos': torch.matmul(cam_coord_directions, torch.tensor([[0.0, 0.0, -1.0]], device=self.device).T)}
        )
