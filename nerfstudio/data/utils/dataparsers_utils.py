...
def get_train_eval_split_interval(image_filenames: List, eval_interval: float) -> Tuple[np.ndarray, np.ndarray]:
    ...
    all_indices = np.arange(num_images) + 1             ############### first image to train
    train_indices = all_indices[all_indices % eval_interval != 0] - 1
    eval_indices = all_indices[all_indices % eval_interval == 0] - 1
