...
from nerfstudio.models.neurodin import NeuRodinModelConfig
...
method_configs["bakedangelo-50w"] = Config(
    method_name="bakedangelo",
    trainer=TrainerConfig(
        steps_per_eval_image=5000,
        steps_per_eval_batch=5000,
...
method_configs["neurodin-stage1-indoor-small"] = Config(
    method_name="neurodin",
    trainer=TrainerConfig(
        steps_per_eval_image=2500,
        steps_per_eval_batch=999999999,
        steps_per_save=20000,
        steps_per_eval_all_images=999999999,  # set to a very large model so we don't eval with all images
    ...
    pipeline=FlexibleInputPipelineConfig(
        datamanager=FlexibleDataManagerConfig(
            ...
            train_num_images_to_sample_from=-1,    # sample from all images
            train_num_times_to_repeat_images=-1,
            num_images_to_sample_pixel_from=-1,
            camera_res_scale_factor=1.0,
            eval_camera_res_scale_factor=0.5,
        ),
        model=NeuRodinModelConfig(
            eval_num_rays_per_chunk=512,
            sdf_field=NeuRodinFieldConfig(
                use_grid_feature=True,
                use_laplace_density=True,
                num_layers=1,
                num_layers_color=4,
                hidden_dim=256,
                hidden_dim_color=256,
                ...
                num_levels=16,
                hash_smoothstep=False,
                enable_spatial_varying_variance=True,
                background_model='none',
            ),
            eikonal_loss_mult=0.01,
            interlevel_loss_mult=1.0,
            rgb_loss_mult=1.0,
            unbias_loss_mask_coeff=0.01,
            unbias_depth_loss_mult=0.05,
            background_model='none',
            use_proposal_network_sampler=True,
            enable_unbias_loss_schedule=True,
            proposal_weights_anneal_max_num_iters=10000,
            enable_variance_schedule=True,
            inv_std_init=0,
            inv_std_end=100,
            scheduled_variance_max_num_iters=250000,
            enable_numerical_gradients_schedule=False,
        ),
    ),
    optimizers={
        "fields": {
            "optimizer": AdamWOptimizerConfig(lr=1e-3, weight_decay=0.01, eps=1e-15),   # we use adamw instead, since neus doesn't use weight decay and we can set weight_decay = 0 which is the same of adam. If weight_decay is on, adam optimizer will result in nan value of mlp output but adamw will correct this.
            "scheduler": MultiStepWarmupSchedulerConfig(warm_up_end=5000, milestones=[160_000, 240_000], gamma=0.1),
        },
        "encoding": {
            "optimizer": AdamWOptimizerConfig(lr=1e-3, weight_decay=0.01, eps=1e-15),
            "scheduler": MultiStepWarmupSchedulerConfig(warm_up_end=5000, milestones=[160_000, 240_000], gamma=0.1),
        ...
        "field_background": {
            "optimizer": AdamWOptimizerConfig(lr=1e-2, weight_decay=0.0, eps=1e-15),
            "scheduler": SchedulerConfig(lr_final=0.0001, max_steps=300000),
        },
        "proposal_networks": {
            "optimizer": AdamOptimizerConfig(lr=1e-2, eps=1e-15),
            "scheduler": MultiStepSchedulerConfig(max_steps=300_000),
...
method_configs["neurodin-stage2-indoor-small"] = Config(
    method_name="neurodin",
    trainer=TrainerConfig(
        steps_per_eval_image=2500,
        steps_per_eval_batch=999999999,
    ...
    pipeline=FlexibleInputPipelineConfig(
        ...
        model=NeuRodinModelConfig(
            ...
            sdf_field=NeuRodinFieldConfig(
                ...
                enable_spatial_varying_variance=True,
                background_model='none',
                use_unbias_for_laplace=True,
            ),
            eikonal_loss_mult=0.01,
            interlevel_loss_mult=1.0,
            rgb_loss_mult=1.0,
            unbias_loss_mask_coeff=0.01,
            unbias_depth_loss_mult=0.0,
            background_model='none',
            use_proposal_network_sampler=True,
            enable_unbias_loss_schedule=False,
            proposal_weights_anneal_max_num_iters=10000,
            enable_variance_schedule=True,
            scheduled_variance_max_num_iters=-1,
            enable_numerical_gradients_schedule=False,
            tangent_normal_consistent_multi=0.005,
        ),
    ),
    optimizers={
        "fields": {
...
method_configs["neurodin-stage1-indoor-large"] = Config(
    method_name="neurodin",
    trainer=TrainerConfig(
        steps_per_eval_image=2500,
        steps_per_eval_batch=999999999,
...
method_configs["neurodin-stage2-indoor-large"] = Config(
    method_name="neurodin",
    trainer=TrainerConfig(
        steps_per_eval_image=2500,
        steps_per_eval_batch=999999999,
...
method_configs["neurodin-stage1-outdoor-large"] = Config(
    method_name="neurodin",
    trainer=TrainerConfig(
        steps_per_eval_image=2500,
        steps_per_eval_batch=999999999,
...
method_configs["neurodin-stage2-outdoor-large"] = Config(
    method_name="neurodin",
    trainer=TrainerConfig(
        steps_per_eval_image=2500,
        steps_per_eval_batch=999999999,
