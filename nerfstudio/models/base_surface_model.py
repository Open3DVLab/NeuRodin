...
class SurfaceModelConfig(ModelConfig):
    ...
    colmap_depth_l1_loss_mult: float = 0.0                  ###########
    """Colmap depth L1 loss multiplier --Yifan"""
...
class SurfaceModel(Model):
    ...
    def populate_modules(self):
        ...
        # self.lpips = LearnedPerceptualImagePatchSimilarity()              ######### won't be necessary
    ...
    def get_loss_dict(self, outputs, batch, metrics_dict=None) -> Dict:
        ...
        ########
        if "rgb_uncertainty" in outputs: 
            render_u_rgb = outputs['rgb_uncertainty']
            # mask = render_u_rgb < self.field.config.uncertainty_thres
            mask = torch.ones_like(render_u_rgb)            ###########
            # mask = ~((outputs['var_near_surface'] < 500)[..., None] * (outputs['rgb_uncertainty'] < 0.4))      # default 100         ############
            loss_dict["rgb_loss"] = self.rgb_loss(mask*(image/render_u_rgb**2), mask*(outputs["rgb"]/render_u_rgb**2)) * self.config.rgb_loss_mult 
            loss_dict['rgb_uncertainty_regularizer'] = torch.mean(torch.log(render_u_rgb**2)) * self.config.rgb_loss_mult           # 0.1 best
            # loss_dict["rgb_loss"] = self.rgb_loss(mask*(image/torch.abs(render_u_rgb)), mask*(outputs["rgb"]/torch.abs(render_u_rgb))) * self.config.rgb_loss_mult 
            # loss_dict['rgb_uncertainty_regularizer'] = torch.mean(torch.log(torch.abs(render_u_rgb))) * self.config.rgb_loss_mult             # 0.1 best
        else:
            loss_dict["rgb_loss"] = self.rgb_loss(image, outputs["rgb"]) * self.config.rgb_loss_mult
            # loss_dict["rgb_loss"] = self.rgb_loss(1. - image, outputs["rgb"]) * self.config.rgb_loss_mult            ### test dark color effect
        ########
        # loss_dict["rgb_loss"] = self.rgb_loss(image, outputs["rgb"]) * self.config.rgb_loss_mult
        if self.training:
            # eikonal loss
            ...
            ###############     should be removed!!!!
            # mask = (outputs['field_outputs']['var'] < 100) * (outputs['field_outputs']['rgb_uncertainty'].reshape(-1, 1) < 0.4)
            # loss_dict["eikonal_loss"] += (((outputs['field_outputs'][FieldHeadNames.GRADIENT].reshape(-1, 3)*mask).norm(2, dim=-1) - 1) ** 2).mean() * self.config.eikonal_loss_mult * 10.0
            ###############
            # s3im loss
            ...
            if "normal" in batch and self.config.mono_normal_loss_mult > 0.0:
                ...
                if "normal_uncertainty" in outputs:
                    render_u_n = outputs['normal_uncertainty']
                    mask = render_u_n < self.field.config.uncertainty_thres
                    loss_dict["normal_loss"] = (
                        F.mse_loss(mask*(normal_pred/render_u_n**2), mask*(normal_gt/render_u_n**2))
                    ) * self.config.mono_normal_loss_mult 
                    loss_dict['normal_uncertainty_regularizer'] = torch.mean(torch.log(render_u_n**2)) * self.config.mono_normal_loss_mult
                else:
                    loss_dict["normal_loss"] = (
                        monosdf_normal_loss(normal_pred, normal_gt) * self.config.mono_normal_loss_mult
                    )

                # mask = (outputs['var_near_surface'] < 100)[..., None] * (outputs['rgb_uncertainty'] < 0.4)      # default 100
                # loss_dict["normal_loss"] = (
                #     monosdf_normal_loss(mask*normal_pred, mask*normal_gt) * self.config.mono_normal_loss_mult
                # )            ##########
            ...
            if "depth" in batch and self.config.mono_depth_loss_mult > 0.0:
                ...
                if "depth_uncertainty" in outputs:
                    ...
                    render_u_d = outputs['depth_uncertainty'].reshape(1, 32, -1)
                    mask = (render_u_d < self.field.config.uncertainty_thres).reshape(1, 32, -1)
                    loss_dict["depth_loss"] = (
                        F.mse_loss(mask*(depth_pred.reshape(1, 32, -1)/render_u_d**2), mask*(depth_gt.reshape(1, 32, -1)/render_u_d**2))
                    ) * self.config.mono_depth_loss_mult 
                    loss_dict['depth_uncertainty_regularizer'] = torch.mean(torch.log(render_u_d**2)) * self.config.mono_depth_loss_mult
                else:
                    mask = torch.ones_like(depth_gt).reshape(1, 32, -1).bool()
                    loss_dict["depth_loss"] = (
                        self.depth_loss(depth_pred.reshape(1, 32, -1), (depth_gt * 50 + 0.5).reshape(1, 32, -1), mask)          ## monocular prior depth value is small ([0.01, 0.04]), scale to close to [0, 1].
                        # self.depth_loss(depth_pred.reshape(1, 32, -1), depth_gt.reshape(1, 32, -1), mask)
                        * self.config.mono_depth_loss_mult
                    )

                # mask = (outputs['var_near_surface'] < 100) * (outputs['rgb_uncertainty'] < 0.4)      
                # scale, shift = compute_scale_and_shift(
                #     depth_pred[None, ...], depth_gt[None, ...], (outputs['var_near_surface'] > 100).bool()[None, :, None]
                #     # depth_pred[None, ...], depth_gt[None, ...], depth_gt[None, ...]>0
                #     )
                # depth_pred = depth_pred * scale + shift
                # loss_dict["depth_loss"] = F.mse_loss(
                #     mask*depth_pred, mask*depth_gt
                # ) * self.config.mono_depth_loss_mult
            ...
            if "colmap_depth" in batch and self.config.colmap_depth_l1_loss_mult > 0.0:
                colmap_depth = batch["colmap_depth"].to(self.device)[..., None]
                depth_pred = outputs["depth"]

                mask = torch.ones_like(colmap_depth).reshape(-1).bool()
                loss_dict["colmap_depth_loss"] = (
                    L1Loss()(depth_pred.reshape(-1)*mask, colmap_depth.reshape(-1)*mask)
                    * self.config.colmap_depth_l1_loss_mult
                )
    ...
    def get_image_metrics_and_images(
        ...
        if "normal_uncertainty" in outputs:                    #############
            normal_uncertainty = colormaps.apply_colormap(outputs['normal_uncertainty'], cmap='turbo')
            images_dict['normal_uncertainty'] = normal_uncertainty

        if "depth_uncertainty" in outputs:
            depth_uncertainty = colormaps.apply_colormap(outputs['depth_uncertainty'], cmap='turbo')
            images_dict['depth_uncertainty'] = depth_uncertainty

        if "rgb_uncertainty" in outputs:
            rgb_uncertainty = colormaps.apply_colormap(outputs['rgb_uncertainty'], cmap='turbo')
            images_dict['rgb_uncertainty'] = rgb_uncertainty

        if "var_near_surface" in outputs:
            tmp = torch.log1p(outputs['var_near_surface'])
            var_near_surface = colormaps.apply_colormap(tmp/tmp.max(), cmap='turbo')
            images_dict['var_near_surface'] = var_near_surface

        if "render_var" in outputs:
            tmp = torch.log1p(outputs['render_var'])
            render_var = colormaps.apply_colormap(tmp/tmp.max(), cmap='turbo')
            images_dict['render_var'] = render_var
